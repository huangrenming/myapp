...
function buildTree(list1) {
  let list = list1.map(i => {
    return {
      ...i
    }
  })
...
function buildTree2(list) {
  ...
  temp2.forEach(item => {
    if (item.parent_id) {
      if (!temp2[findIndex(item.parent_id)].children) {
        temp2[findIndex(item.parent_id)].children = new Array();
      }
      temp2[findIndex(item.parent_id)].children.push(item)
    } else {
      tree2.push(item)
    }
  })
...
var tree = buildTree(menu_list)
var tree2 = buildTree2(menu_list)

// console.log(JSON.stringify(tree))
export default { tree, tree2 }
